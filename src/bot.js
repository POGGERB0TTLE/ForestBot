...
require('events').EventEmitter.defaultMaxListeners = 19;
const mineflayer = require("mineflayer"),
  ...
  query = require("./data/querys.json"),
  fs = require("fs"),
  path = require("path"),
  tab = require('./misc/tab.js'),
  sleep = require('./misc/sleep.js'),
...
let channel = channels.mainLiveChat
let HOST = config.mcServer;
...
client.on("message", async (m) => {
  ...
  channel.forEach((chnl) => {
    ...
    if (m.content.includes('\n')) return m.reply("Sorry, your message can't be that long.")
...
bot.on("chat:MSGpluginRegex", (content) => { //for whisper messages using the MSG plugin
  const user = content[0][0], message = content[0][1];
...
bot.on("chat:whisperFrom", (content) => { //whisper from event
  const user = content[0][0], message = content[0][1];
  if (message.startsWith(`${prefix}say`)) {
    let split = message.split(' ').slice(1).join(' ');
    bot.chat(split);
  }
  return embed(pink, `${user} » ${message}`);
});
