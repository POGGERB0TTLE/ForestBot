...
bot.addChatPattern("whisperTo", /^\[me -> ([^ ]*)\] (.*)$/, { parse: true }); //[me -> ForestBot] message
bot.addChatPattern("whisperFrom", /^\[([^ ]*) -> me\] (.*)$/, { parse: true }); //[ForestBot -> me] message
bot.addChatPattern("whisperTo", /^You whisper to ([^ ]*): (.*)$/, {
  parse: true,
}); //You whisper to ForestBot: message
bot.addChatPattern("whisperFrom", /^([^ ]*) whispers: (.*)$/, { parse: true }); //ForestBot whispers: message
bot.addChatPattern("whisperTo", /^\[You -> ([^ ]*)\] (.*)$/, { parse: true }); // [You -> Febzey] message
bot.addChatPattern("whisperFrom", /^\[([^ ]*) -> You\] (.*)$/, { parse: true }); // [Febzey -> You] message
