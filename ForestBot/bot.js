...
let channel = config.mainLiveChat,
...
const embed = async (c, m) => {
  //main way of sending messages to discord from bot
...
client.on("message", (m) => {
  ...
  // if(m.content.startsWith(`${config.prefix}setbridge`)) {
  ...
  //   let StringedArgs = arguments.toString();
  ...
  //   config.mainLiveChat.push(StringedArgs)
  //   return;
  // }
  // }
...
bot.on("chat:chat", async (a) => {
  //minecraft bot main chat listener
  embed(grey, `**${a[0][0]}** » ${a[0][1]}`);
});

const playtime = async () => {
  //playtime recorder for each player online (saves every minute)
...
async function saveUser(player) {
  //saving new users who join the server
...
bot.once("login", async () => {
  ...
  require("./patterns.js"); //loading all of our custom chat patterns.
  require("./tab.js");
  setInterval(playtime, 60000); //incrementing playtime every minute.
});

setTimeout(function () {
  //player joined/leave events
...
bot.on("chat:teleport", async (content) => {
  //tpa ForestBot
  const a = content[0][0];
...
bot.on("chat:pve", async (a) => {
  //when someone dies from environment
  let v = a[0][0];
  db.query(query.updateDeaths, [v]);
  return embed(purple, `${a[0].input}`);
});

bot.on("chat:pvp", async (a) => {
...
bot.on("chat:MSGpluginRegex", (content) => {
  //for whisper messages using the MSG plugin
  const user = content[0][0],
    message = content[0][1];
  return embed(pink, `${user} » ${message}`);
});

bot.on("chat:whisperTo", (content) => {
  //whipser to event
  const user = content[0][0],
    message = content[0][1];
  return embed(pink, `${user} » ${message}`);
});
bot.on("chat:whisperFrom", (content) => {
  //whisper from event
  const user = content[0][0],
    message = content[0][1];
  return embed(pink, `${user} » ${message}`);
...
bot.on("error", (e) => {
  //if bot gets an error
  return embed(orange, `**${e}**`);
});
bot.on("kicked", (e) => {
  //when the bot gets kicked from the server..
  return embed(orange, `**${e}**`);
});
bot.on("end", async () => {
  //when bot leaves
